...
def check_integrity_experiment(experiment_path):
    ...
    if 'Experiment Info.csv' in files:
        check_experiment_info_file(os.path.join(experiment_path, 'Experiment Info.csv'))

    if 'All Recordings.csv' in files:
        check_all_recordings_file(os.path.join(experiment_path, 'All Recordings.csv'))

    if 'All Tracks.csv' in files:
        check_all_tracks_file(os.path.join(experiment_path, 'All Tracks.csv'))

    if 'All Squares.csv' in files:
        check_all_squares_file(os.path.join(experiment_path, 'All Squares.csv'))
...
def check_all_recordings_file(file):
    ...
    if phase_1_incomplete:
        paint_logger.error(
            f"All Recordings file '{file}' is missing columns for Phase 1: {phase_1_incomplete}")

    phase_2_incomplete = set(expected_columns_1 + expected_columns_2) - set(actual_columns)
    if phase_2_incomplete:
        paint_logger.error(
            f"All Recordings file '{file}' is missing columns for Phase 2: {phase_2_incomplete}")

    excess_columns = set(actual_columns) - set(expected_columns_1) - set(expected_columns_2)
    if excess_columns :
        paint_logger.error(
            f"All Recordings file '{file}' has excess columns: {excess_columns}")
...
def check_all_tracks_file(file):
    ...
    if set(expected_columns) - set(actual_columns):
        paint_logger.error(
            f"All Tracks file '{file}' is missing columns: {set(expected_columns) - set(actual_columns)}")
    if set(actual_columns) - set(expected_columns):
        paint_logger.error(
            f"All Tracks file '{file}' has excess columns: {set(actual_columns) - set(expected_columns)}")
...
if __name__ == '__main__':
    ...
    experiment_path = '/Users/hans/Paint Demo Set/Paint Demo'
    check_integrity_project(experiment_path)