...
With Pycharm installed, the environment set up, and the code cloned from the repository you are ready to run the pipeline. There is a collection of scripts and support files, but for just tuning the pipeline, a small number of scripts are needed. They are found in the src directory under Applications
