...
read_data <- function(root_directory, perc_tracks = 0.1) {
...
td <- squares_master %>%
  filter(Probe %in% c('1 Mono', '6 Mono', '1 Tri',  '6 Tri'),
         Density_Ratio > 2,
         R_Squared > 0.8,
         Tau > 0,
         Adjuvant == 'None')
...
generate_and_customize_plot <- function(cell_type, probe_type, y_limit, customize_y_axis = FALSE) {
  plot_data <- 
    td %>% filter(Cell_Type == cell_type, Probe_Type == probe_type)
...
generate_and_customize_plot('BMDC', 'Simple', 600, FALSE) + 
